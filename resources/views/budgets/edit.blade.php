        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="mb-8">
                <div class="bg-gradient-to-r from-indigo-600 to-purple-700 rounded-2xl shadow-xl p-6 text-white flex items-center justify-between">
                    <h1 class="text-2xl font-bold">Edit Pagu Tahunan</h1>
                    ...
                    <form method="POST" action="{{ route('budgets.update', $budget) }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" name="year" value="{{ old('year', $budget->year) }}" class="mt-1 block w-full rounded-lg border-gray-300" required>
                                @error('year')<p class="text-sm text-red-600 mt-1">{{ $message }}</p>@enderror
                            </div>
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 mb-2">Nama</label>
                                <input type="text" name="name" value="{{ old('name', $budget->name) }}" class="mt-1 block w-full rounded-lg border-gray-300" required>
                                @error('name')<p class="text-sm text-red-600 mt-1">{{ $message }}</p>@enderror
                            </div>
                            <div class="md:col-span-2">
                                <label class="block text-sm font-semibold text-gray-700 mb-2">Jumlah Pagu</label>
                                <input type="number" step="0.01" name="amount" value="{{ old('amount', $budget->amount) }}" class="mt-1 block w-full rounded-lg border-gray-300" required>
                                @error('amount')<p class="text-sm text-red-600 mt-1">{{ $message }}</p>@enderror
                            </div>
                            <div class="md:col-span-2">
                                <label class="block text-sm font-semibold text-gray-700 mb-2">Deskripsi</label>
                                <textarea name="description" class="mt-1 block w-full rounded-lg border-gray-300" rows="3">{{ old('description', $budget->description) }}</textarea>
