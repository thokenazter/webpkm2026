        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="mb-8">
                <div class="bg-gradient-to-r from-indigo-600 to-purple-700 rounded-2xl shadow-xl p-6 text-white flex items-center justify-between">
                    <div>
                        <h1 class="text-2xl font-bold mb-1">Master RAB - Kegiatan</h1>
                        <p class="text-indigo-100">Kelola daftar Kegiatan per Rincian Menu</p>
                    </div>
                    <a href="{{ route('rab-kegiatans.create') }}" class="inline-flex items-center bg-white text-indigo-700 px-4 py-2 rounded-lg font-medium hover:bg-indigo-50 shadow-sm"><i class="fas fa-plus mr-2"></i>Tambah</a>
                </div>
            </div>
                        ...
                        <table class="min-w-full table-auto">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach ($kegiatans as $kg)
                                    @php
                                        $compKey = $kg->menu?->component_key;
                                        $compName = $compKey ? (\App\Models\Rab::components()[$compKey] ?? $compKey) : null;
                                        $componentBadges = [
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-6 py-3 text-sm">
                                            <a href="{{ route('rab-kegiatans.show', $kg) }}" class="inline-flex items-center text-indigo-600 hover:text-indigo-900 mr-3"><i class="fas fa-eye mr-1"></i>Lihat</a>
                                            <a href="{{ route('rab-kegiatans.edit', $kg) }}" class="inline-flex items-center text-yellow-600 hover:text-yellow-900 mr-3"><i class="fas fa-edit mr-1"></i>Edit</a>
                                            <form action="{{ route('rab-kegiatans.destroy', $kg) }}" method="POST" class="inline" onsubmit="return confirm('Yakin?')">
                                                @csrf @method('DELETE')
