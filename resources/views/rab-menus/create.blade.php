        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="mb-8">
                <div class="bg-gradient-to-r from-indigo-600 to-purple-700 rounded-2xl shadow-xl p-6 text-white flex items-center justify-between">
                    <h1 class="text-2xl font-bold">Tambah Rincian Menu</h1>
            ...
            <div class="bg-white overflow-hidden shadow-lg rounded-2xl border border-gray-100">
                <div class="p-6">
                    <form method="POST" action="{{ route('rab-menus.store') }}">
                        @csrf
                        <div class="grid grid-cols-1 gap-4">
                            <div>
                                ...
                                <select name="component_key" class="mt-1 block w-full rounded-lg border-gray-300" required>
                                    ...
                                    @foreach ($components as $key => $label)
                                        <option value="{{ $key }}" {{ old('component_key') === $key ? 'selected' : '' }}>{{ $label }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <input type="text" name="name" value="{{ old('name') }}" class="mt-1 block w-full rounded-lg border-gray-300" required>
