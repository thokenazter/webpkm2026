...
@section('title', 'Berita & Kegiatan - Puskesmas')
            ...
            <div class="lg:grid lg:grid-cols-4 lg:gap-8">
                    ...
                    <div class="sticky top-24 card p-6">
                        ...
                        <nav class="space-y-2">
                            <a href="{{ route('berita.index') }}" 
                               class="block px-4 py-2 rounded-lg text-sm font-medium transition-colors {{ !request('kategori') ? 'bg-primary-100 text-primary-700' : 'text-neutral-600 hover:bg-neutral-100' }}">
                                Semua
                            </a>
                            @foreach($categories as $category)
                                <a href="{{ route('berita.index', ['kategori' => $category->slug]) }}" 
                                   class="flex items-center justify-between px-4 py-2 rounded-lg text-sm font-medium transition-colors {{ request('kategori') === $category->slug ? 'bg-primary-100 text-primary-700' : 'text-neutral-600 hover:bg-neutral-100' }}">
                                    {{ $category->name }}
                                    <span class="text-xs bg-neutral-100 px-2 py-0.5 rounded-full">{{ $category->posts_count }}</span>
                                </a>
                            @endforeach
                        </nav>
                    </div>
                ...
                <div class="lg:hidden mb-6 overflow-x-auto">
                    <div class="flex gap-2 pb-2">
                        <a href="{{ route('berita.index') }}" 
                           class="whitespace-nowrap px-4 py-2 rounded-full text-sm font-medium transition-colors {{ !request('kategori') ? 'bg-primary-600 text-white' : 'bg-neutral-100 text-neutral-600' }}">
                            Semua
                        </a>
                        @foreach($categories as $category)
                            <a href="{{ route('berita.index', ['kategori' => $category->slug]) }}" 
                               class="whitespace-nowrap px-4 py-2 rounded-full text-sm font-medium transition-colors {{ request('kategori') === $category->slug ? 'bg-primary-600 text-white' : 'bg-neutral-100 text-neutral-600' }}">
                                {{ $category->name }}
                            </a>
                        @endforeach
                    </div>
                        ...
                        <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
                            @foreach($posts as $post)
                                <article class="card overflow-hidden group">
                                    <a href="{{ route('berita.show', $post) }}" class="block">
                                        ...
                                        <div class="p-5">
                                            <time class="text-xs text-neutral-500">{{ $post->formatted_date }}</time>
