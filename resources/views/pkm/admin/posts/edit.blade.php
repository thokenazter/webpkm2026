@extends('pkm.admin.layouts.app')

@section('title', 'Edit Berita')
@section('page-title', 'Edit Berita')

@section('content')
    <form action="{{ route('pkm-admin.posts.update', $post) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="card p-6">
                    ...
                    <textarea id="excerpt" 
                              ...
                              placeholder="Ringkasan singkat untuk preview (opsional)...">{{ old('excerpt', $post->excerpt) }}</textarea>
                </div>
            </div>
            ...
            <div class="space-y-6">
                ...
                <div class="card p-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="pt-3 border-t border-neutral-200">
                            <p class="text-xs text-neutral-500">
                                <i data-lucide="eye" class="w-3 h-3 inline"></i> {{ number_format($post->views_count) }} views
                            </p>
                            <p class="text-xs text-neutral-500 mt-1">
                                <i data-lucide="calendar" class="w-3 h-3 inline"></i> Dibuat: {{ $post->created_at->format('d M Y, H:i') }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="flex gap-3 mt-6 pt-4 border-t border-neutral-200">
                        ...
                        <a href="{{ route('pkm-admin.posts.index') }}" class="btn border border-neutral-300 text-neutral-600 hover:bg-neutral-100">
                            Batal
                        </a>
                    </div>
                </div>
                    ...
                    <div class="space-y-4">
                        @if($post->featured_image)
                            <div id="current-image">
                                <img src="{{ $post->featured_image_url }}" alt="{{ $post->title }}" class="w-full h-40 object-cover rounded-lg">
                                <p class="text-xs text-neutral-500 mt-2">Gambar saat ini</p>
                            </div>
                        @endif
                        ...
                        <div class="border-2 border-dashed border-neutral-300 rounded-lg p-6 text-center hover:border-primary-500 transition-colors" id="dropzone">
                            ...
                            <label for="featured_image" class="cursor-pointer">
                                ...
                                <p class="text-sm text-neutral-600">{{ $post->featured_image ? 'Ganti gambar' : 'Upload gambar' }}</p>
...
ClassicEditor
    ...
    .then(editor => {
        ...
        // Sync CKEditor content to textarea on change
