...
<head>
    ...
    <title>@yield('title', 'Admin Panel') - Puskesmas</title>
            ...
            <nav class="px-4 py-6 space-y-1 overflow-y-auto h-[calc(100vh-180px)]">
                <a href="{{ route('pkm-admin.dashboard') }}"
                    class="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors
                          {{ request()->routeIs('pkm-admin.dashboard') ? 'bg-primary-600 text-white' : 'text-neutral-300 hover:bg-neutral-800 hover:text-white' }}">
                ...
                <a href="{{ route('pkm-admin.posts.index') }}"
                    class="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors
                          {{ request()->routeIs('pkm-admin.posts.*') ? 'bg-primary-600 text-white' : 'text-neutral-300 hover:bg-neutral-800 hover:text-white' }}">
                    <i data-lucide="newspaper" class="w-5 h-5"></i>
                    Berita
                </a>

                <a href="{{ route('pkm-admin.galleries.index') }}"
                    class="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors
                          {{ request()->routeIs('pkm-admin.galleries.*') ? 'bg-primary-600 text-white' : 'text-neutral-300 hover:bg-neutral-800 hover:text-white' }}">
                    <i data-lucide="images" class="w-5 h-5"></i>
                    Galeri
                </a>

                <a href="{{ route('pkm-admin.categories.index') }}"
                    class="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors
                          {{ request()->routeIs('pkm-admin.categories.*') ? 'bg-primary-600 text-white' : 'text-neutral-300 hover:bg-neutral-800 hover:text-white' }}">
                    <i data-lucide="tags" class="w-5 h-5"></i>
                    Kategori
                </a>

                <a href="{{ route('pkm-admin.messages.index') }}"
                    class="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors
                          {{ request()->routeIs('pkm-admin.messages.*') ? 'bg-primary-600 text-white' : 'text-neutral-300 hover:bg-neutral-800 hover:text-white' }}">
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <header class="bg-white border-b border-neutral-200 px-4 lg:px-8 py-4">
                <div class="flex items-center justify-between">
                    ...
                    {{-- Page Title --}}
                    <h1 class="text-xl font-semibold text-neutral-900 hidden lg:block">@yield('page-title', 'Dashboard')
                    </h1>
